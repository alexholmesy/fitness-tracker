...
import { upsertWater } from '@/lib/actions'
...
export function WaterClient() {
  ...
  const logQuick = async (litres: number) => {
    setLoading(true)
    try {
      await upsertWater({ date: format(new Date(), 'yyyy-MM-dd'), litres: litres.toString() })
      router.refresh()
      setOpen(false)
    } finally {
      setLoading(false)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await upsertWater({
        ...
        litres: fd.get('litres') as string,
      })
      setOpen(false)
      router.refresh()
    } finally {
