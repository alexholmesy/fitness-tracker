...
export default async function WorkoutsPage({ searchParams }: PageProps) {
  ...
  const totalVolume = workouts.reduce((total, w) => {
    const vol = (w.workout_exercises ?? []).reduce((s, e) => {
      ...
      <div className="px-4 grid grid-cols-3 gap-3">
        ...
        <div className="stat-card text-center">
          ...
          <p className="text-xl font-bold">
            {workouts.length > 0 ? Math.round(workouts.length / (dateRange === '7d' ? 1 : dateRange === '30d' ? 4.3 : 12.9) * 10) / 10 : 0}
          </p>
        </div>
        <div className="stat-card text-center">
      ...
      <div className="px-4 space-y-3">
        {workouts.length === 0 ? (
          ...
          workouts.map(workout => {
            ...
            const volume = exercises.reduce((s, e) => s + (e.sets ?? 0) * (e.reps ?? 0) * (e.weight ?? 0), 0)
            return (
