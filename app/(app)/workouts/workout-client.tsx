...
type Exercise = { exercise_name: string; sets: string; reps: string; weight: string }
const emptyExercise = (): Exercise => ({ exercise_name: '', sets: '', reps: '', weight: '' })

export function WorkoutClient({ onSave }: { onSave: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data: workout, error: workoutError } = await supabase
        .from('workouts')
        .insert({ user_id: user.id, name: fd.get('name') as string, date: fd.get('date') as string, notes: (fd.get('notes') as string) || null })
        .select().single()

      if (workoutError) throw workoutError

      const validExercises = exercises.filter(e => e.exercise_name.trim()).map((e, i) => ({
      ...
      if (validExercises.length > 0) {
        await supabase.from('workout_exercises').insert(validExercises)
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setExercises(prev => [...prev, emptyExercise()])} className="flex items-center gap-1 text-xs text-primary font-medium press-effect">
            ...
            {exercises.map((ex, i) => (
              <div key={i} className="bg-secondary rounded-xl p-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {exercises.length > 1 && (
                    <button type="button" onClick={() => setExercises(prev => prev.filter((_, idx) => idx !== i))} className="text-destructive/60 hover:text-destructive press-effect">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  )}
                </div>
                <input value={ex.exercise_name} onChange={e => updateExercise(i, 'exercise_name', e.target.value)} placeholder="e.g. Bench Press"
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['sets', 'reps', 'weight'] as const).map(field => (
                    <input key={field} value={ex[field]} onChange={e => updateExercise(i, field, e.target.value)}
                      placeholder={field === 'weight' ? 'kg' : field} type="number" step={field === 'weight' ? '0.5' : '1'}
