...
export default function WeightPage() {
  ...
  const refetch = async () => {
    ...
    const days = range === '7d' ? 7 : range === '30d' ? 30 : range === '90d' ? 90 : range === '1y' ? 365 : 3650
    const fromDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    const { data } = await supabase.from('weight_entries').select('*').eq('user_id', user.id).gte('date', fromDate).order('date', { ascending: true })
    setEntries(data ?? [])
  }
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <PageHeader
        ...
        action={<WeightClient entries={entries} onSave={refetch} />}
      />
      ...
      <div className="px-4 space-y-2">
        ...
        {loading ? (
          ...
          <WeightHistoryList entries={entries} onDelete={refetch} />
        )}
      </div>
    </div>
  )
