...
export default function RoutinePage() {
     ...
     supabase
       ...
       .then(({ data }) => {
         if (data) {
           ...
           setNotes(data.notes ?? '')
           setExercises(
             (data.routine_exercises ?? []).sort((a: any, b: any) => a.sort_order - b.sort_order)
           )
         }
         setLoading(false)
       })
 ...
 return (
   <div className="pb-6 animate-fade-in">
       ...
       <div className="space-y-2">
         ...
         {exercises.length === 0 && (
           <div className="stat-card text-center py-6">
             <p className="text-sm text-muted-foreground">No exercises yet. Tap below to add.</p>
           </div>
         )}
         ...
         {exercises.map((ex, idx) => (
           <div key={idx} className="stat-card space-y-2">
             <div className="flex items-center justify-between">
               <p className="text-sm font-semibold flex-1 min-w-0 truncate mr-2">{ex.exercise_name}</p>
             ...
             <div className="grid grid-cols-2 gap-2">
               <div className="space-y-1">
                 ...
                 <input
                   ...
                   onChange={e => setExercises(prev => prev.map((e2, i) =>
                     i === idx ? { ...e2, target_sets: parseInt(e.target.value) } : e2
                   ))}
               ...
               <div className="space-y-1">
                 ...
                 <input
                   ...
                   onChange={e => setExercises(prev => prev.map((e2, i) =>
                     i === idx ? { ...e2, target_reps: parseInt(e.target.value) } : e2
                   ))}
                   className="w-full px-3 py-2 rounded-xl bg-secondary border border-border text-sm text-center focus:outline-none focus:ring-1 focus:ring-primary/50"
                 />
               </div>
     ...
     {/* Exercise picker — fixed height, search pinned */}
     {showPicker && (
